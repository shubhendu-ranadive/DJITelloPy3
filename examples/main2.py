...
def main():
	...
	time.sleep(3)
	response = drone.get_response()
	if response is None:
		print("Connection Problem! Check connection to WiFi again.")
		return 0

    #battery = drone.get_battery()
	print('Battery: %s' % (drone.get_battery()))
	if drone.get_battery() < 30:
		print("Low Battery! Please charge the drone.")
		return 0
	...
	try:
		while True:
			...
			if key == 27:					# k が27(ESC)だったらwhileループを脱出，プログラム終了
				...
				drone.send_command('rc 0 15 15 -40')
				time.sleep(20)
				drone.send_command('rc 0 0 0 0')	# ラジコン指令をゼロに
				time.sleep(2)
				drone.land()	# 着陸
				...
				drone.move_up(0.5)
				time.sleep(3)
				drone.send_command('rc 15 0 -25 0')
				time.sleep(4)
				drone.send_command('rc 15 0 25 0')
				time.sleep(2)
				drone.send_command('rc -15 0 25 0')
			...
			#print("Height: %d" %(drone.get_height()))
